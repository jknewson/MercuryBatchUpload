    ...
    'MainViewModel', 'toastr','bootstrap','datepicker'], function (koo, vm, toastr) {
        ...
        ko.extenders.detectionLimitValidation = function (target, option) {
            ...
            function validate() {
                ...
                if (target.Method() == null) {
                    ...
                    if (target() < target.Method().method_detection_limit) detectionFlag = '<';
                    else if (target.detection_limit < target() && target.value() > target.Method().method_detection_limit) detectionFlag = 'e';
                    else detectionFlag = 'none';
                    warn = true;
        ...
        ko.bindingHandlers.datepicker = {
            init: function (element, valueAccessor, allBindingsAccessor) {
                ...
                (<any>$el).datepicker(options);

                //handle the field changing
                ko.utils.registerEventHandler(element, "change", function () {
                    var observable = valueAccessor();
                    observable((<any>$el).datepicker("getDate"));
                });

                //handle disposal (if KO removes by the template binding)
                ko.utils.domNodeDisposal.addDisposeCallback(element, function () {
                    (<any>$el).datepicker("remove");
                });
            ...
            update: function (element, valueAccessor) {
                ...
                var current = (<any>$(element)).datepicker("getDate");

                if (value - current !== 0) {
                    (<any>$(element)).datepicker("setDate", value);
                }
            }
        };
